...
{
    public class TemporaryInstallationsMudsHistoryDao : AbstractManagedDao, ITemporaryInstallationsMudsHistoryDao
    {
        private const string QUERY_HISTORIES_BY_ID = "QueryFormMudsTemporaryInstallationHistoryById";
        private const string INSERT = "InsertFormMudsTemporaryInstallationHistory";

        private readonly IUserDao userDao;

        public TemporaryInstallationsMudsHistoryDao()
        {
            userDao = DaoRegistry.GetDao<IUserDao>();
        }

        public List<TemporaryInstallationsMudsHistory> GetById(long id)
        {
            SqlCommand command = ManagedCommand;
            command.AddParameter("@Id", id);
            return command.QueryForListResult<TemporaryInstallationsMudsHistory>(PopulateInstance, QUERY_HISTORIES_BY_ID);
        }

        public void Insert(TemporaryInstallationsMudsHistory history)
        {
            SqlCommand command = ManagedCommand;
            command.Insert(history, AddInsertParameters, INSERT);
        }

        private void AddInsertParameters(TemporaryInstallationsMudsHistory history, SqlCommand command)
        {
        ...
        private TemporaryInstallationsMudsHistory PopulateInstance(SqlDataReader reader)
        {
            ...
            return new TemporaryInstallationsMudsHistory(id, formStatus, functionalLocations, plainTextContent, validFromDateTime,
