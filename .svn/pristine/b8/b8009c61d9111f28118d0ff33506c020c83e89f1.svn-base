IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'QueryLogDTOsByWorkPermitMontreal')
    BEGIN
        DROP PROCEDURE [dbo].QueryLogDTOsByWorkPermitMontreal
    END
GO

CREATE Procedure [dbo].QueryLogDTOsByWorkPermitMontreal
    (
...
FROM
	...
	INNER JOIN [LogWorkPermitMontrealAssociation] assoc on assoc.LogId = l.Id
...
GRANT EXEC ON QueryLogDTOsByWorkPermitMontreal TO PUBLIC
GO