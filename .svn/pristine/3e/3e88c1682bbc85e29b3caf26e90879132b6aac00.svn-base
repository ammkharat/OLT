...
{
    public class LubesCsdFormDTODao : AbstractManagedDao, ILubesCsdFormDTODao
    {
        private const string QUERY_CSD_BY_FLOCS_AND_OTHER_THINGS =
            "QueryFormLubesCsdDTOsByFunctionalLocationsAndOtherThings";

        private const string QUERY_APPROVED_CSD_BY_FLOCS =
            "QueryFormDTOsThatAreLubesCsdApprovedDraftExpiredByFunctionalLocations";

        private const string QUERY_EXPIRED_OR_APPROVED_AND_ACTIVE_CSD_BY_FLOCS =
            "QueryFormDTOsThatAreLubesCsdExpiredOrApprovedAndActiveByFunctionalLocations";

        public List<LubesCsdFormDTO> QueryFormCsd(IFlocSet flocSet, DateRange dateRange,
            IEnumerable<FormStatus> formStatuses,
            bool includeAllDraftFormsRegardlessOfDateRange)
        {
            ...
            return GetDtos(command, QUERY_CSD_BY_FLOCS_AND_OTHER_THINGS);
        }

        public List<LubesCsdFormDTO> QueryFormCsdsThatAreApprovedDraftExpiredByFunctionalLocations(IFlocSet flocSet,
            DateTime now)
        {
            var command = ManagedCommand;
            command.AddParameter("@CsvFlocIds", flocSet.FunctionalLocations.BuildIdStringFromList());
            command.AddParameter("@Now", now);
            return GetDtos(command, QUERY_APPROVED_CSD_BY_FLOCS);
        }

        public List<LubesCsdFormDTO> QueryFormCsdsThatAreExpiredOrApprovedAndActiveByFunctionalLocations(IFlocSet flocSet,
            DateTime now)
        {
            var command = ManagedCommand;
            command.AddParameter("@CsvFlocIds", flocSet.FunctionalLocations.BuildIdStringFromList());
            command.AddParameter("@Now", now);
            return GetDtos(command, QUERY_EXPIRED_OR_APPROVED_AND_ACTIVE_CSD_BY_FLOCS);
        }

        private static List<LubesCsdFormDTO> GetDtos(SqlCommand command, string query)
        {
            var result = new Dictionary<long, LubesCsdFormDTO>();
            ...
            return new List<LubesCsdFormDTO>(result.Values);
        }
        ...
        private static LubesCsdFormDTO PopulateInstance(SqlDataReader reader)
        {
            var id = GetId(reader);
            ...
            var systemBeingDefeated = reader.Get<string>("CriticalSystemDefeated");
            ...
            var result =
                new LubesCsdFormDTO(id, floc, criticalSystemDefeated, createdByUserId,
                    ...
                    hasBeenApproved, lastModifiedUser, systemBeingDefeated);
