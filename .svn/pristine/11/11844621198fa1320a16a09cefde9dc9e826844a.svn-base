...
{
    public class MontrealCsdDTODao : AbstractManagedDao, IMontrealCsdDTODao
    {
        private const string QUERY_MONTREAL_CSD_BY_FLOCS_AND_OTHER_THINGS =
            "QueryFormMontrealCsdDTOsByFunctionalLocationsAndOtherThings";

        private const string QUERY_APPROVED_MONTREAL_CSD_BY_FLOCS =
            "QueryFormDTOsThatAreMontrealCsdApprovedDraftExpiredByFunctionalLocations";

        public List<MontrealCsdDTO> QueryFormMontrealCsd(IFlocSet flocSet, DateRange dateRange,
        ...
        public List<MontrealCsdDTO> QueryFormMontrealCsdsThatAreApprovedDraftExpiredByFunctionalLocations(
        ...
        private static List<MontrealCsdDTO> GetDtos(SqlCommand command, string query)
        {
            var result = new Dictionary<long, MontrealCsdDTO>();
            ...
            return new List<MontrealCsdDTO>(result.Values);
        }
        ...
        private static MontrealCsdDTO PopulateInstance(SqlDataReader reader)
        {
            var id = GetId(reader);
            ...
            var result =
                new MontrealCsdDTO(id, new List<string> {floc}, criticalSystemDefeated, createdByUserId,
