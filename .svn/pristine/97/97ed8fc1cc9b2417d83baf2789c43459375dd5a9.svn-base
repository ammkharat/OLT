...
{
    public partial class EditFormDropdownsConfigurationForm : BaseForm, IEditFormDropdownsConfigurationForm
    {
        private DomainSummaryGrid<DropdownValue> grid;
        private List<DropdownValue> valuesList;

        public EditFormDropdownsConfigurationForm(List<DropdownValue> dropdownValues, string key,
            string nameAlreadyExistsErrorMessage)
        {
            InitializeComponent();
            InitializeGrid();
            InitializePresenter(dropdownValues, key, nameAlreadyExistsErrorMessage);
        }
        ...
        public DropdownValue LaunchAddEditValueForm(FormDropdown dropdown, DropdownValue editObject,
            List<DropdownValue> masterList, string nameAlreadyExistsErrorMessage)
        {
            var form = new AddEditFormDropdownValueForm(dropdown, editObject, masterList, nameAlreadyExistsErrorMessage);
            return form.ShowDialogAndReturnValue(this);
        }

        private void InitializeGrid()
        {
            var gridHeader = StringResources.FormEdmontonDropdownConfiguration_ValuesGridHeader;
            grid = new DomainSummaryGrid<DropdownValue>(new SingleColumnGridRenderer(gridHeader, "Name"),
        ...
        private void InitializePresenter(List<DropdownValue> dropdownValues, string key,
            string nameAlreadyExistsErrorMessage)
        {
            var presenter = new EditFormDropdownsConfigurationFormPresenter(this, dropdownValues, key,
