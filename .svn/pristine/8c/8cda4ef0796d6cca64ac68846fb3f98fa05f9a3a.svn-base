if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[InsertWorkPermitUSPipeline]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	drop procedure [dbo].[InsertWorkPermitUSPipeline]
GO

Create Procedure [dbo].[InsertWorkPermitUSPipeline]
(
    @Id bigint Output,
	...
	(
		...
		from 
			...
			inner join functionallocation 
				ON workpermitUSPipeline.functionallocationid = functionallocation.[id] 
			where
...
INSERT INTO WorkPermitUSPipeline
(
	Version,
    LastModifiedDate,
    LastModifiedUserId,
...
go
GRANT EXEC ON InsertWorkPermitUSPipeline TO PUBLIC
GO