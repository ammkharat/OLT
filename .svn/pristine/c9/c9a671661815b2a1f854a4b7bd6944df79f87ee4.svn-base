...
{
    public class WorkPermitMudsPagePresenter : AbstractDeletableDomainPagePresenter<WorkPermitMudsDTO, WorkPermitMuds, IWorkPermitMudsDetails, IWorkPermitMudsPage>
    {
        ...
        public WorkPermitMudsPagePresenter()
            : base(new WorkPermitMudsPage())
        {
        ...
        private void Print(object sender, EventArgs args)
        {
            if (ClientSession.GetUserContext().SiteConfiguration.EnableWorkPermitSignature)
            {
                ...
                else if (workPermitSign.ChangeStatus == true)
                {
                    // Added By Vibhor : RITM0556998 - Add new status signed

                    WorkPermitMuds wpMuds = QueryForFirstSelectedItem();
                    wpMuds.VERIFICATEUR_Text = workPermitSign.VERIFICATEUR_Text;
                    wpMuds.EMETTEUR_Text = workPermitSign.EMETTEUR_Text;
                    wpMuds.DETENTEUR_Text = workPermitSign.DETENTEUR_Text;
                    wpMuds.GasResultat_Text = workPermitSign.GasResultat_Text;

                    if (wpMuds.VERIFICATEUR_Text != string.Empty || wpMuds.EMETTEUR_Text != string.Empty ||
                        wpMuds.DETENTEUR_Text != string.Empty || wpMuds.GasResultat_Text != string.Empty)
                    {
                        wpMuds.WorkPermitStatus = PermitRequestBasedWorkPermitStatus.Signed;
                        ServiceEventDispatcher.CallServiceAndDispatchImmediateEventNotification(workPermitMudsService.Update, wpMuds); 
                    }
        ...
        protected override void ControlDetailButtons()
        {
            ...
            List<WorkPermitMudsDTO> selectedItems = page.SelectedItems;
            ...
            //details.CloseEnabled = hasItemsSelected;
            ...
            //Added by Mukesh
            if (ClientSession.GetUserContext().SiteConfiguration.EnableWorkPermitSignature && page.SelectedItems.Count > 1)
            {
                details.PrintEnabled = false;
            }

            if(page.SelectedItems.Count==1 && page.SelectedItems[0].WorkPermitMudsType.Id==WorkPermitMudsType.ELEVATED_HOT.Id && page.SelectedItems[0].Status!=PermitRequestBasedWorkPermitStatus.Complete )
            {
                if (
                page.SelectedItems[0].Status==PermitRequestBasedWorkPermitStatus.Complete ||
                page.SelectedItems[0].Status==PermitRequestBasedWorkPermitStatus.Void || 
                page.SelectedItems[0].Status==PermitRequestBasedWorkPermitStatus.Incomplete || 
                page.SelectedItems[0].Status==PermitRequestBasedWorkPermitStatus.CompletionUnknown || 
                page.SelectedItems[0].Status==PermitRequestBasedWorkPermitStatus.NoShow || 
                page.SelectedItems[0].Status==PermitRequestBasedWorkPermitStatus.NotReturned || 
                page.SelectedItems[0].Status==PermitRequestBasedWorkPermitStatus.OnHold || 
                page.SelectedItems[0].Status==PermitRequestBasedWorkPermitStatus.MissingInformation 
                )
                    
                {
                    details.GasTestButtonEnabled = false;
                }
                else
                {
                    details.GasTestButtonEnabled = true;
                }
...
// Added By Vibhor : DMND0010816 -MUDS - Implement OLT Work permit
        ...
        protected override WorkPermitMudsDTO CreateDTOFromDomainObject(WorkPermitMuds domainObject)
        {
            return new WorkPermitMudsDTO(domainObject);
        }
         ...
         private void MarkAsTemplate(object sender, EventArgs e)
         {
             ...
             else
             {
                 ...
                 if (workPermit.TemplateName != string.Empty && nameForm.Save == true)
                 {
                     ...
                     ServiceEventDispatcher.CallServiceAndDispatchImmediateEventNotification(workPermitMudsService.InsertTemplate, workPermit);
                 }
                 else
                 {
