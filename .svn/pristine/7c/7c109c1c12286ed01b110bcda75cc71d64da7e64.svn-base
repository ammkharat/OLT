IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'QueryFormDocumentSuggestionByFunctionalLocations')
    BEGIN
        DROP PROCEDURE [dbo].QueryFormDocumentSuggestionByFunctionalLocations
    END
GO

CREATE Procedure [dbo].QueryFormDocumentSuggestionByFunctionalLocations
    (
...
WHERE 
	(f.ValidFromDateTime <= @EndOfDateRange AND f.MaxEndDateTime >= @StartOfDateRange )
ORDER BY f.Id
OPTION (OPTIMIZE FOR UNKNOWN)	
GO

GRANT EXEC on QueryFormDocumentSuggestionByFunctionalLocations TO PUBLIC
GO