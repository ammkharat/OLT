IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'QueryFormDTOsThatAreLubesAlarmDisableApprovedDraftExpiredByFunctionalLocations')
    BEGIN
        DROP PROCEDURE [dbo].QueryFormDTOsThatAreLubesAlarmDisableApprovedDraftExpiredByFunctionalLocations
    END
GO

CREATE Procedure [dbo].QueryFormDTOsThatAreLubesAlarmDisableApprovedDraftExpiredByFunctionalLocations
    (
...
WHERE 
    FormStatusId = 2 OR FormStatusId = 5 OR FormStatusId = 1
ORDER BY f.Id, ApprovalDisplayOrder
OPTION (OPTIMIZE FOR UNKNOWN)
GO

GRANT EXEC on QueryFormDTOsThatAreLubesAlarmDisableApprovedDraftExpiredByFunctionalLocations TO PUBLIC
GO