IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'QueryShiftHandoverQuestionnaireDTOsWithYesAnswersByFunctionalLocationAndDateRange')
    BEGIN
        DROP PROCEDURE [dbo].QueryShiftHandoverQuestionnaireDTOsWithYesAnswersByFunctionalLocationAndDateRange
    END
GO

  
CREATE Procedure [dbo].QueryShiftHandoverQuestionnaireDTOsWithYesAnswersByFunctionalLocationAndDateRange  
    (  
...
select distinct q.id  
  ...
  WHERE   
    ((q.CreatedDateTime >= @StartDateTime and  
 q.CreatedDateTime <= @EndDateTime  and IsFlexible=0)      
/*RITM RITM0185797 Flexi shift handover */      
/*Amit Shukla start*/      
 OR 
 ((( @StartDateTime BETWEEN  q.FlexiShiftStartDate AND q.FlexiShiftEndDate) OR ( @EndDateTime BETWEEN  q.FlexiShiftStartDate AND q.FlexiShiftEndDate)) and IsFlexible=1))     
...
GRANT EXEC ON QueryShiftHandoverQuestionnaireDTOsWithYesAnswersByFunctionalLocationAndDateRange TO PUBLIC
GO 