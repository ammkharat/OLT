...
{
    public class WorkPermitMudsPage : AbstractPage<WorkPermitMudsDTO, IWorkPermitMudsDetails>, IWorkPermitMudsPage
    {
        public WorkPermitMudsPage()
            : base(new DomainSummaryGrid<WorkPermitMudsDTO>(
            new WorkPermitMudsGridRenderer(), OltGridAppearance.MULTI_SELECT_WRAPPED_TEXT, "workPermitPageGrid"),
        ...
        protected override bool IsCreatedByCurrentUser(WorkPermitMudsDTO permit)
        {
            return (permit != null && permit.CreatedByUserId == ClientSession.GetUserContext().User.Id);
        }

        protected override bool IsUpdatedByCurrentUser(WorkPermitMudsDTO permit)
        {
