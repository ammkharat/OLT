...
{
    public class WorkPermitEdmontonValidator
    {
        private readonly WorkPermitEdmontonBaseValidationAdapter adapter;
        private readonly LabelAttributes attributesForHazardsLabel;
        private bool hasErrors;
        private bool hasWarnings;

        public WorkPermitEdmontonValidator(WorkPermitEdmontonBaseValidationAdapter adapter,
        ...
        public void ValidateAndSetErrors(DateTime nowInEdmonton)
        {
            ClearErrors();
        ...
        protected void ValidateHasAtLeastOneSafetyRequirementIfSectionIsEnabled()
        {
            if (!adapter.WorkersMinimumSafetyRequirementsSectionNotApplicableToJob)
            {
                var values = new List<bool>
                {
                    ...
                    adapter.Goggles,
                    adapter.RubberBoots,
                    adapter.RubberGloves,
                    adapter.RubberSuit,
                    adapter.SafetyHarnessLifeline,
                    adapter.HighVoltagePPE,
                    adapter.Other1,
                    adapter.EquipmentGrounded,
                    adapter.FireBlanket,
                    adapter.FireExtinguisher,
                    ...
                    adapter.BreathingAirApparatus,
                    adapter.DustMask,
                    adapter.LifeSupportSystem,
                    adapter.SafetyWatch,
                    adapter.ContinuousGasMonitor,
                    adapter.WorkersMonitor,
                    adapter.BumpTestMonitorPriorToUse,
                    adapter.Other3,
                    adapter.AirMover,
                    adapter.BarriersSigns,
                    adapter.RadioChannel,
                    adapter.AirHorn,
                    adapter.MechVentilationComfortOnly,
                    adapter.AsbestosMMCPrecautions,
                    adapter.Other4
                };
        ...
        protected void ValidateOtherSafetyRequirementsAreFilledInIfTheyAreChecked()
        {
            if (!adapter.WorkersMinimumSafetyRequirementsSectionNotApplicableToJob)
            {
                ...
                if (adapter.Other4 && adapter.Other4Value.IsNullOrEmptyOrWhitespace())
                {
                    adapter.ActionForOther4CheckedWithNoValue();
                    SetHasWarning();
                }
            }
        }
        ...
        {
            if (
                !DevExpressMeasurementUtility.StringWillFitIntoField(attributesForHazardsLabel,
        ...
        private void ValidateRadioNumber()
        {
            if (adapter.RadioChannel && adapter.RadioChannelNumber.IsNullOrEmptyOrWhitespace())
            {
                adapter.ActionForNoRadioChannelNumber();
                SetHasWarning();
            }
        }

        private void ValidateMonitorNumber()
        {
            if (adapter.WorkersMonitor && adapter.WorkersMonitorNumber.IsNullOrEmptyOrWhitespace())
            {
                adapter.ActionForNoWorkersMonitorNumber();
                SetHasWarning();
            }
        }
        ...
        private void ValidateAtLeastOneFireProtectiveMeasureIsSelectedIfPermitTypeIsHighEnergy()
        {
            if (adapter.WorkPermitType != null &&
                adapter.WorkPermitType.Equals(WorkPermitEdmontonType.HIGH_ENERGY_HOT_WORK))
            {
        ...
        private void ValidateAtLeastOneIssuedToOptionIsSelected()
        {
            if (adapter.IssuedToContractor == false && adapter.IssuedToSuncor == false)
            {
                adapter.ActionForNoIssuedToOptionSelected();
                SetHasWarning();
            }
        }
        ...
        private bool GasTestLineIsValid(
            string gasTestDataLineCombustibleGas, string gasTestDataLineOxygen, string gasTestDataLineToxicGas,
            Time gasTestDataLineTime)
        {
            if (GasTestLineIsEmpty(gasTestDataLineCombustibleGas, gasTestDataLineOxygen, gasTestDataLineToxicGas,
                gasTestDataLineTime))
            {
                return true;
            }
            ...
            {
                gasTestDataLineCombustibleGas,
                gasTestDataLineOxygen,
                gasTestDataLineToxicGas,
                gasTestDataLineTime == null ? null : gasTestDataLineTime.NullableToString()
            };

            return !stringValues.Exists(value => value.IsNullOrEmptyOrWhitespace());
        }

        public static bool GasTestLineIsEmpty(string gasTestDataLineCombustibleGas, string gasTestDataLineOxygen,
            string gasTestDataLineToxicGas, Time gasTestDataLineTime)
        {
            return (gasTestDataLineCombustibleGas.IsNullOrEmptyOrWhitespace() &&
                    gasTestDataLineOxygen.IsNullOrEmptyOrWhitespace() &&
                    gasTestDataLineToxicGas.IsNullOrEmptyOrWhitespace() &&
                    gasTestDataLineTime == null);
        }
        ...
        private void ValidateRequestedAndExpiryDateTimes(DateTime nowInEdmonton)
        {
            ...
            if (expiryDateTime < nowInEdmonton)
            {
