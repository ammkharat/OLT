...
{
    public class WorkPermitMontrealTemplatePagePresenter : AbstractDeletableDomainPagePresenter<WorkPermitMontrealDTO, WorkPermitMontreal, IWorkPermitMontrealDetails, IWorkPermitMontrealPage>
    {
        ...
        public WorkPermitMontrealTemplatePagePresenter()
            : base(new WorkPermitMontrealTemplatePage())
        {
        ...
        protected override void ControlDetailButtons()
        {
            ...
            details.DeleteVisible = hasItemsSelected;// && authorized.ToDeleteWorkPermits(userRoleElements, selectedItems);
            ...
            //hasSingleItemSelected && authorized.ToEditWorkPermit(userRoleElements, selectedItems[0]);
...
//Added By Vibhor : RITM0613645 : OLT - Template Easy clone **Delete Feature**
            ...
            List<WorkPermitMontrealDTO> permitDtos = page.SelectedItems;
            try
            {
                workPermit.LastModifiedBy = ClientSession.GetUserContext().User;
                workPermit.LastModifiedDateTime = Clock.Now;
                if (permitDtos.Count == 1)
                {
                    ...
                    ServiceEventDispatcher.CallServiceAndDispatchImmediateEventNotification(workPermitMontrealService.RemoveTemplate, workPermit);
                }
            ...
            if (permitDtos.Count == 1)
            {
                ...
                else
                {
                    if (workPermit.TemplateName != string.Empty)
                    {
                        ...
                        workPermit.IsTemplate = true;
                        workPermit.TemplateCreatedBy = ClientSession.GetUserContext().User.Username;
                        ServiceEventDispatcher.CallServiceAndDispatchImmediateEventNotification(workPermitMontrealService.UpdateTemplate, workPermit);
                    }
                    else
                    {
                        workPermit.IsTemplate = false;
