...
{
    public abstract class AbstractFormFortHillsFormPresenter<TForm, TFormView> : AddEditBaseFormPresenter<TFormView, TForm>
        where TForm : BaseFortHillsForm
        where TFormView : class, IFormView
    {
        private bool saveWasSuccessful;

        protected AbstractFormFortHillsFormPresenter(TFormView view, TForm editObject)
        ...
        protected void SaveWithApprovalCheckForFortHillsForm7And59(bool showEmail, string buttontext) // Swapnil Patki For DMND0005325 Point Number 7
        {
            UpdateEditObjectFromView();
            ...
            else
            {
                ...
                if (userContext.IsSarniaSite) //&& buttontext != "saveButton")     //ayman sarnia
                {
                    if (buttontext == null)
                    {
                        var shiftSupervisorApproval =
                            editObject.AllApprovals.FindAll(
                                approval =>
                                    approval.Approver.ToLower().StartsWith("shift supervisor") &&
                                    approval.IsApproved &&
                                    approval.Enabled);
                        ...
                        {
                            if (shiftSupervisorApproval.Count > 0 && Clock.Now > editObject.FromDateTime.AddDays(3) || (!editObject.AllApprovalsAreIn() && Clock.Now >= editObject.FromDateTime && shiftSupervisorApproval.Count == 0)) 
