...
{
    public class MudsTemporaryInstallationsTimerManager
    {
        private static readonly ILog logger = LogManager.GetLogger(typeof(MudsTemporaryInstallationsTimerManager));
        ...
        public void RegisterTimer(TemporaryInstallationsMudsDTO mudsTemporaryInstallationFormDTO, TimeSpan dueTime, TimerCallback callback)
        {
            ...
            Timer timer = new Timer(callback, mudsTemporaryInstallationFormDTO, dueTimeInMilliseconds, Timeout.Infinite);
            DateTime approximateFireTime = Clock.Now.AddMilliseconds(dueTimeInMilliseconds);

            try
            {
                locker.AcquireWriterLock(Timeout.Infinite);
                bool itemAlreadyExisted = RemoveFromTimersDictionary(mudsTemporaryInstallationFormDTO.IdValue);
                AddToTimersDictionary(mudsTemporaryInstallationFormDTO.IdValue, timer, approximateFireTime);

                if (itemAlreadyExisted)
                {
                    logger.WarnFormat("OP!$ {0} already existed in Timer Dictionary. So it was removed first. This should not be happening.", mudsTemporaryInstallationFormDTO.IdValue);
                }

                logger.DebugFormat("Adding timer to change grouping of muds temporary installation form id {0} in {1} seconds", mudsTemporaryInstallationFormDTO.IdValue, dueTime.TotalSeconds);
            }
            finally
            {
        ...
        public void Unregister(TemporaryInstallationsMudsDTO mudsTemporaryInstallationFormDTO)
        {
            try
            {
                locker.AcquireWriterLock(Timeout.Infinite);

                if (timers.ContainsKey(mudsTemporaryInstallationFormDTO.IdValue))
                {
                    Timer timer = timers[mudsTemporaryInstallationFormDTO.IdValue];
                    InvalidateAndDisposeTimer(timer);
                    RemoveFromTimersDictionary(mudsTemporaryInstallationFormDTO.IdValue);
                }
            }
            finally
            {
