IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'QueryLogDTOsByWorkPermitEdmonton')
    BEGIN
        DROP PROCEDURE [dbo].QueryLogDTOsByWorkPermitEdmonton
    END
GO

CREATE Procedure [dbo].QueryLogDTOsByWorkPermitEdmonton
    (
...
FROM
	...
	INNER JOIN [LogWorkPermitEdmontonAssociation] assoc on assoc.LogId = l.Id
...
WHERE
    l.deleted = 0 AND
    l.LogType = 1 AND
	assoc.WorkPermitEdmontonId = @WorkPermitEdmontonId
GO

GRANT EXEC ON QueryLogDTOsByWorkPermitEdmonton TO PUBLIC
GO