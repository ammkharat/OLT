...
IF NOT EXISTS (
  SELECT * 
  FROM   INFORMATION_SCHEMA.COLUMNS --sys.columns 
  WHERE TABLE_NAME='GenericTemplateEFormHeader'
  AND COLUMN_NAME='isNeverEnd'
    
)
begin
ALTER TABLE [dbo].[GenericTemplateEFormHeader] ADD isNeverEnd bit NOT NULL DEFAULT 0
