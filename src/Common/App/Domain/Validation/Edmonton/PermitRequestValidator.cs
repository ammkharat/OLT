...
namespace Com.Suncor.Olt.Common.Domain.Validation.Edmonton
{
    ...
    {
        private readonly PermitRequestBaseValidationAdapter adapter;
        ...
        public PermitRequestValidator(PermitRequestBaseValidationAdapter adapter, DataSource dataSource)
        {
        ...
        protected virtual void ValidateHasAtLeastOneSafetyRequirementIfSectionIsEnabled()
        {
        ...
        protected virtual void ValidateOtherSafetyRequirementsAreFilledInIfTheyAreChecked()
        {
            if (!adapter.WorkersMinimumSafetyRequirementsSectionNotApplicableToJob)
            {
                if (adapter.Other1Selected && adapter.Other1Text.IsNullOrEmptyOrWhitespace())
                {
                    adapter.ActionForOther1SelectedWithNoValue();
                    SetHasError();
                }

                if (adapter.Other2Selected && adapter.Other2Text.IsNullOrEmptyOrWhitespace())
                {
                    adapter.ActionForOther2SelectedWithNoValue();
                    SetHasError();
                }

                if (adapter.Other3Selected && adapter.Other3Text.IsNullOrEmptyOrWhitespace())
                {
                    adapter.ActionForOther3SelectedWithNoValue();
                    SetHasError();
                }

                if (adapter.Other4Selected && adapter.Other4Text.IsNullOrEmptyOrWhitespace())
                {
                    adapter.ActionForOther4SelectedWithNoValue();
                    SetHasError();
                }
            }
        }
