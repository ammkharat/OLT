...
{
    [Serializable]
    public abstract class BaseFortHillsForm : ModifiableDomainObject, IFunctionalLocationRelevant, IFortHillsForm
    {
        private List<FormApproval> approvals = new List<FormApproval>();
        private FormStatus formStatus;

       
        protected BaseFortHillsForm(long? id, FormStatus formStatus, DateTime validFromDateTime, DateTime validToDateTime,
      ...
      //  public abstract FortHillsFormType FormType { get; }
       ...
       // public abstract IFormFortHillsDTO CreateDTO();
        ...
        public virtual void ConvertToClone(User createdByUser)
        {
            ...
            FromDateTime = now;
            SetDefaultDatesBasedOnShift(WorkPermitFortHills.IsDayShift(now.ToTime()), now.ToDate(), now.ToTime());
        ...
        public bool IsRelevantTo(long siteIdOfClient, 
            ...
            List<string> workPermitFortHillsFullHierarchies, 
            ...
            SiteConfiguration siteConfiguration)
        {
            ...
            if (siteConfiguration.FormsFlocSetType.Equals(FunctionalLocationSetType.WorkPermit) &&
                !workPermitFortHillsFullHierarchies.IsEmpty())
            {
                fullHierarchies = workPermitFortHillsFullHierarchies;
            }
            else
            {
        ...
        {
            var validToTime = WorkPermitFortHills.NightShiftStartTime;
        ...
        public bool IsWorkPermitDatesWithinFormDates(WorkPermitFortHills workPermitFortHills)
        {
            var formRange = new Range<DateTime>(FromDateTime, ToDateTime);
            return formRange.ContainsInclusive(workPermitFortHills.RequestedStartDateTime) &&
                   formRange.ContainsInclusive(workPermitFortHills.ExpiredDateTime);
        }

        //mangesh - clone permit request
        public bool IsWorkPermitDatesWithinFormDates(PermitRequestFortHills workPermitFortHills)
        {
            //DateTime s = Date.ToDateTimeOrMaxValue(workPermitFortHills.RequestedStartDate);
    ...
    public interface IFortHillsForm
    {
        //FortHillsFormType FormType { get; }
        long FormNumber { get; }
