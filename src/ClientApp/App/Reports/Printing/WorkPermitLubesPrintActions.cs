...
{
    public class WorkPermitLubesPrintActions : PrintActions<WorkPermitLubes, WorkPermitLubesReport, WorkPermitLubesReportAdapter>
    {
        private readonly IWorkPermitLubesService workPermitService;
        private readonly bool printEmptyForm;

        public WorkPermitLubesPrintActions(IWorkPermitLubesService workPermitService) : this(workPermitService, false)
        {           
        }

        public WorkPermitLubesPrintActions(IWorkPermitLubesService workPermitService, bool printEmptyForm)
        {
            this.workPermitService = workPermitService;
            this.printEmptyForm = printEmptyForm;
        }

        protected override WorkPermitLubesReport CreateSpecificReport()
        {
            return new WorkPermitLubesReport();
        }

        protected override List<WorkPermitLubesReportAdapter> CreateReportAdapter(WorkPermitLubes workPermit)
        {
            WorkPermitLubesReport report = new WorkPermitLubesReport(); // temp for doing some calculations
        ...
        protected override void AddPageSpecificWatermarks(WorkPermitLubesReport report, IEnumerable<WorkPermitLubesReportAdapter> adapters)
        {
            int index = 0;
        ...
        protected override ReportPrintPreference CreateReportPrintPreference(WorkPermitLubesReport report, UserPrintPreference userPrintPreferences)
        {
