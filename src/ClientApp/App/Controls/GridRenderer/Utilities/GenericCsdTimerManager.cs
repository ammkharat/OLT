...
{
    public class GenericCsdTimerManager
    {
        private static readonly ILog logger = LogManager.GetLogger(typeof(GenericCsdTimerManager));
        ...
        public void RegisterTimer(GenericCsdDTO montrealCsdFormDTO, TimeSpan dueTime, TimerCallback callback)
        {
            ...
            try
            {
                ...
                logger.DebugFormat("Adding timer to change grouping of csd id {0} in {1} seconds", montrealCsdFormDTO.IdValue, dueTime.TotalSeconds);
            }
            finally
            {
        ...
        public void Unregister(GenericCsdDTO montrealCsdFormDTO)
        {
            try
            {
                locker.AcquireWriterLock(Timeout.Infinite);
