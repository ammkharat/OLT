...
{
    public class ActionItemPagePresenter : AbstractRespondableDomainPagePresenter<ActionItemDTO, ActionItem, IActionItemDetails, IActionItemPage>
    {
        ...
        public ActionItemPagePresenter()  : this(new ActionItemPage())
        {
        }

        public ActionItemPagePresenter(IActionItemPage page)  : this(
            page,
        ...
        protected ActionItemPagePresenter(
            ...
            : base(page, authorized, remoteEventRepeater, objectLockingService, timeService, userService)
        {
            ...
            page.Details.GoToDefinitionVisible = authorized.ToViewActionItemDefinitions(ClientSession.GetUserContext().UserRoleElements);
...
//RITM0467567 : Added by Vibhor OLT - Adding Pictures on Action item and Directives
            //details.actionItemImage = actionItem.Imagelist;
        ...
        protected override IList<ActionItemDTO> GetDtos(Range<Date> range)
        {
            ...
            var noreadings = service.QueryDTOsByFunctionalLocationsAndDateRange(userContext.RootFlocSet, ActionItemStatus.AvailableForCurrentView, range, userContext.ReadableVisibilityGroupIds);
            if (noreadings != null && noreadings.Count > 0)
            {
                List<ActionItemDTO> ListToRemove = new List<ActionItemDTO>();
                foreach (ActionItemDTO aidto in noreadings)
                {
