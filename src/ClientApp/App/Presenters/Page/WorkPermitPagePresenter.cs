...
namespace Com.Suncor.Olt.Client.Presenters.Page
{
    public class WorkPermitPagePresenter : AbstractApprovableDomainPagePresenter<WorkPermitDTO, WorkPermit, IWorkPermitDetails, IWorkPermitPage>
    {
        ...
        public WorkPermitPagePresenter()
            : this(new WorkPermitPage())
        {
        }

        public WorkPermitPagePresenter(IWorkPermitPage page)
            : this(
                page,
        ...
        protected override void Edit(WorkPermit domainObject)
        {
            ...
            //ayman USPipeline workpermit
                        ...
                        userContext.IsSELCSite && WorkPermit.IsOldVersionForUSPipeline(domainObject.Version)) // mangesh uspipeline to selc
            {
                DialogResult dialogResult = OltMessageBox.Show(page.ParentForm, StringResources.WorkPermit_CannotEditOldFormat, StringResources.WorkPermit_CannotEditOldFormat_Title, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dialogResult == DialogResult.Yes)
                {
        ...
        public void Print(object sender, EventArgs args)
        {
            ...
            {
                // Added by Vibhor : RITM0617220 - OLT - Sarnia - adding a pop-up to remind the permit issuer that additional signatures may be required

                WorkPermit wp = QueryForFirstSelectedItem();

                if ((wp.Attributes.IsConfinedSpaceEntry || 
                    wp.Attributes.IsBurnOrOpenFlame ||
                    wp.IsCoauthorizationRequired == true ||
                    wp.Attributes.IsAsbestos ||
                    wp.AdditionItemsRequired.IsElectrical == true ) &&
                   (wp.WorkPermitStatus != WorkPermitStatus.Issued || wp.WorkPermitStatus != WorkPermitStatus.Issued ))
                {

                    SarniaMessagePopupForm form = new SarniaMessagePopupForm();
                    form.ShowDialog();
        ...
        private void Print()
        {
            ...
            if (ClientSession.GetUserContext().IsUSPipelineSite || ClientSession.GetUserContext().IsSELCSite) // mangesh us[pipeline to selc
            {
        ...
        protected override void ControlDetailButtons()
        {
            ...
            if (ClientSession.GetUserContext().IsUSPipelineSite       //ayman USPipeline workpermit
                    || ClientSession.GetUserContext().IsSELCSite)  // mangesh uspipeline to selc
            {
                bool bothtrue = false;
            ...
            if (userContext.IsSarniaSite || userContext.IsDenverSite)
            {
                ...
                details.ScanbuttonButtonVisible = hasSingleItemSelected && userRoleElements.HasRoleElement(RoleElement.WORKPERMIT_SCAN);

                if (details.ScanbuttonButtonVisible == true)
                {
                    details.viewAttachementbuttonVisible = true;
                }
                else
                {
        ...
        private void MarkAsTemplate(object sender, EventArgs e)
        {
            ...
            else
            {
                if (workPermit.TemplateName != string.Empty && nameForm.Save == true)
                {
                    ...
                    ServiceEventDispatcher.CallServiceAndDispatchImmediateEventNotification(workPermitService.InsertTemplate, workPermit);
                }
                else
                {
