...
from
(
select SummaryLogId, l.WorkAssignmentId, STUFF 
( 
  (SELECT @SplittingDelimiter + innerLcfe.SummaryLogCustomFieldName
    FROM SummaryLogCustomFieldEntry innerLcfe
    where innerLcfe.SummaryLogId = lcfe.SummaryLogId
    order by innerLcfe.SummaryLogCustomFieldName
    FOR XML PATH(''), root('MyString'), type
  ).value('/MyString[1]','varchar(max)') 
,1,len(@SplittingDelimiter),'') as customFieldNamesList
from SummaryLogCustomFieldEntry lcfe
inner join [SummaryLog] l on l.Id = SummaryLogId
group by SummaryLogId, l.WorkAssignmentId
) t
group by WorkAssignmentId, customFieldNamesList
order by c
...
begin
  ...
  where
	cfgwa.WorkAssignmentId = @WorkAssignmentId
	and cfg.AppliesToSummaryLogs = 1
  ...
  if (@MatchCount = 0)
    begin
		...
		begin
			...
			select @DisplayOrder = lcfe.DisplayOrder
			from SummaryLogCustomFieldEntry lcfe
			inner join @LogIds li on li.Id = lcfe.SummaryLogId
			where lcfe.SummaryLogCustomFieldName = @TmpCustomFieldName
		...
		from SummaryLogCustomFieldEntry lcfe
		inner join @LogIds li on li.Id = lcfe.SummaryLogId
		inner join CustomField cf on cf.Name = lcfe.SummaryLogCustomFieldName
		inner join @TmpCustomFieldIds cfids on cfids.Id = cf.Id
	...
	*/  
    begin
	    ...
	    from SummaryLogCustomFieldEntry lcfe
	    inner join @LogIds li on li.Id = lcfe.SummaryLogId
		inner join @CustomFieldsThatWouldBeOnALogCreatedToday cf on cf.CustomFieldName = lcfe.SummaryLogCustomFieldName
	end
