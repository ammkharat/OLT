IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'QueryDeviationAlertDTOsByFLOCIdsWithOverlapInDateRange')
	BEGIN
		DROP PROCEDURE [dbo].QueryDeviationAlertDTOsByFLOCIdsWithOverlapInDateRange
	END
GO
  
Create Procedure [dbo].QueryDeviationAlertDTOsByFLOCIdsWithOverlapInDateRange  
(  
    @Ids varchar(MAX),  
 @FromDate datetime,  
 @ToDate datetime  
...
WHERE  
 ...
 AND NOT (da.StartDateTime >= @ToDate or  
          da.EndDateTime <= @FromDate)  
ORDER BY  
    FunctionalLocationId  
OPTION (OPTIMIZE FOR UNKNOWN)   

GRANT EXEC ON QueryDeviationAlertDTOsByFLOCIdsWithOverlapInDateRange TO PUBLIC