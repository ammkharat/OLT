IF EXISTS (SELECt * FROM sysobjects WHERE type = 'P' AND name = 'QueryLogDefinitionDTOByUserRootFlocDirectAncestorsAndDescendants')
    BEGIN
        DROP PROCEDURE [dbo].QueryLogDefinitionDTOByUserRootFlocDirectAncestorsAndDescendants
    END
GO

CREATE Procedure [dbo].QueryLogDefinitionDTOByUserRootFlocDirectAncestorsAndDescendants
    (
...
WHERE
	...
	(
      ...
      (
		EXISTS
		(
        -- Floc of Log Definition matches one of the passed in flocs
        select lfl.LogDefinitionId From IDSplitter(@CsvFLOCIds) ids
        INNER JOIN LogDefinitionFunctionalLocation lfl ON ids.Id = lfl.FunctionalLocationId
        WHERE lfl.LogDefinitionId = ld.Id
		)
        OR EXISTS
		(
        -- Floc of Log Definition is parent of one of the passed in flocs (look up the floc tree from my selected flocs)
		...
		OR EXISTS
		(
        -- Floc of Log Definition is child of one of the passed in flocs (look down the floc tree from my selected flocs)
        select lfl.LogDefinitionId from FunctionalLocationAncestor a
...
GRANT EXEC ON QueryLogDefinitionDTOByUserRootFlocDirectAncestorsAndDescendants TO PUBLIC
GO