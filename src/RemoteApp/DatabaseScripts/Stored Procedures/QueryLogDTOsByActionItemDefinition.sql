IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'QueryLogDTOsByActionItemDefinition')
    BEGIN
        DROP PROCEDURE [dbo].QueryLogDTOsByActionItemDefinition
    END
GO

CREATE Procedure [dbo].QueryLogDTOsByActionItemDefinition
    (
...
FROM
	...
	LEFT JOIN [LogActionItemDefinitionAssociation] laida ON laida.LogId = l.Id
...
WHERE
    l.deleted = 0 AND
    l.LogType = 1 AND
	laida.ActionItemDefinitionId = @ActionItemDefinitionId
GO

GRANT EXEC ON [QueryLogDTOsByActionItemDefinition] TO PUBLIC
GO