IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'QueryLogDTOsByTargetAlert')
    BEGIN
        DROP PROCEDURE [dbo].QueryLogDTOsByTargetAlert
    END
GO

CREATE Procedure [dbo].QueryLogDTOsByTargetAlert
    (
...
FROM
	...
	INNER JOIN [LogTargetAlertAssociation] assoc on assoc.LogId = l.Id
...
WHERE
    l.deleted = 0 AND
    l.LogType = 1 AND
	assoc.TargetAlertId = @TargetAlertId
GO

GRANT EXEC ON QueryLogDTOsByTargetAlert TO PUBLIC
GO