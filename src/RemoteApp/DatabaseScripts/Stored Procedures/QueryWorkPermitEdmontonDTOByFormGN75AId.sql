IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'QueryWorkPermitEdmontonDTOByFormGN75AId')
    BEGIN
        DROP PROCEDURE [dbo].QueryWorkPermitEdmontonDTOByFormGN75AId
    END
GO

CREATE Procedure [dbo].QueryWorkPermitEdmontonDTOByFormGN75AId
    (
		@FormGN75AId bigint
    )
AS
...
WHERE
	wped.FormGN75AId = @FormGN75AId AND
	wp.Deleted = 0

GRANT EXEC ON QueryWorkPermitEdmontonDTOByFormGN24Id TO PUBLIC
GO