IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'QueryWorkPermitEdmontonDTOByFormGN1Id')
    BEGIN
        DROP PROCEDURE [dbo].QueryWorkPermitEdmontonDTOByFormGN1Id
    END
GO

CREATE Procedure [dbo].QueryWorkPermitEdmontonDTOByFormGN1Id
    (
		@FormGN1Id bigint
    )
AS
...
WHERE
	wped.FormGN1Id = @FormGN1Id AND
	wp.Deleted = 0

GRANT EXEC ON QueryWorkPermitEdmontonDTOByFormGN1Id TO PUBLIC
GO