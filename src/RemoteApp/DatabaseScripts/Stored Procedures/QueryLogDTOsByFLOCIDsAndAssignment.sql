IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'QueryLogDTOsByFLOCIDsAndAssignment')
    BEGIN
        DROP PROCEDURE [dbo].QueryLogDTOsByFLOCIDsAndAssignment
    END
GO

CREATE Procedure [dbo].QueryLogDTOsByFLOCIDsAndAssignment
    (
...
WHERE
    ...
    l.CreatedDateTime >= @StartOfDateRange AND
	  (
		  (@WorkAssignmentId is null AND l.WorkAssignmentId is null) OR
		  (@WorkAssignmentId = l.WorkAssignmentId)
    ) AND
	(
...
FROM
    [Log] l
    INNER JOIN Log_Id_CTE ON l.Id = Log_Id_CTE.LogId
...
GRANT EXEC ON [QueryLogDTOsByFLOCIDsAndAssignment] TO PUBLIC
GO