IF EXISTS (SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[InsertFormGN75BNew]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)  

DROP PROCEDURE dbo.InsertFormGN75BNew
...
GO
CREATE Procedure [dbo].[InsertFormGN75BNew]
(
	@Id bigint Output,
	@FormStatusId int,
...
BEGIN
	select top 1 @NewFormId = id from FormGN75B order by id desc
