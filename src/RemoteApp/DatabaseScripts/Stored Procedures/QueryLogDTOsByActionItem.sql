IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'QueryLogDTOsByActionItem')
    BEGIN
        DROP PROCEDURE [dbo].QueryLogDTOsByActionItem
    END
GO

CREATE Procedure [dbo].QueryLogDTOsByActionItem
    (
	    @ActionItemId bigint
    )
AS
...
FROM
	...
	LEFT OUTER JOIN [LogActionItemAssociation] laia ON laia.LogId = l.Id
...
WHERE
    l.deleted = 0 AND
    l.LogType = 1 AND
	laia.ActionItemId = @ActionItemId
GO

GRANT EXEC ON QueryLogDTOsByActionItem TO PUBLIC
GO