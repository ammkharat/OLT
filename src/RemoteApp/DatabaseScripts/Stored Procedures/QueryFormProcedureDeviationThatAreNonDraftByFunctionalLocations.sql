IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'QueryFormProcedureDeviationThatAreNonDraftByFunctionalLocations')
    BEGIN
        DROP PROCEDURE [dbo].QueryFormProcedureDeviationThatAreNonDraftByFunctionalLocations
    END
GO

CREATE Procedure [dbo].QueryFormProcedureDeviationThatAreNonDraftByFunctionalLocations
(
...
WHERE 
	(f.ValidToDateTime >= @Now ) AND
	-- Approved(2), RevisionInProgress(8), Cancelled (4), Complete(14)
	(f.FormStatusId = 2 OR f.FormStatusId = 8 OR 
	f.FormStatusId = 4 OR f.FormStatusId = 14)
ORDER BY f.Id
OPTION (OPTIMIZE FOR UNKNOWN)	
GO

GRANT EXEC on QueryFormProcedureDeviationThatAreNonDraftByFunctionalLocations TO PUBLIC
GO