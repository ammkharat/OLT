...
{
    public class GenericCsdHistoryDao : AbstractManagedDao, IGenericCsdHistoryDao
    {
        private const string QUERY_HISTORIES_BY_ID = "QueryFormGenericCsdHistoryById";
        private const string INSERT = "InsertFormGenericCsdHistory";

        private readonly IUserDao userDao;

        public GenericCsdHistoryDao()
        {
            userDao = DaoRegistry.GetDao<IUserDao>();
        }

        public List<GenericCsdHistory> GetById(long id)
        {
            SqlCommand command = ManagedCommand;
            command.AddParameter("@Id", id);
            return command.QueryForListResult<GenericCsdHistory>(PopulateInstance, QUERY_HISTORIES_BY_ID);
        }

        public void Insert(GenericCsdHistory history)
        {
            SqlCommand command = ManagedCommand;
            command.Insert(history, AddInsertParameters, INSERT);
        }

        private void AddInsertParameters(GenericCsdHistory history, SqlCommand command)
        {
        ...
        private GenericCsdHistory PopulateInstance(SqlDataReader reader)
        {
            ...
            return new GenericCsdHistory(id, formStatus, functionalLocations, plainTextContent, validFromDateTime,
