...
{
    public class WorkPermitFortHillsHistoryDao : AbstractManagedDao, IWorkPermitFortHillsHistoryDao
    {
        private const string QUERY_HISTORIES_BY_ID = "QueryWorkPermitFortHillsHistoryById";
        private const string INSERT = "InsertWorkPermitFortHillsHistory";

        private readonly IUserDao userDao;

        public WorkPermitFortHillsHistoryDao()
        {
            userDao = DaoRegistry.GetDao<IUserDao>();            
        }

        public List<WorkPermitFortHillsHistory> GetById(long id)
        {
            SqlCommand command = ManagedCommand;
            command.AddParameter("@Id", id);
            return command.QueryForListResult<WorkPermitFortHillsHistory>(PopulateInstance , QUERY_HISTORIES_BY_ID);
        }

        public void Insert(WorkPermitFortHillsHistory history)
        {
            SqlCommand command = ManagedCommand;
            command.Insert(history, AddInsertParameters, INSERT);
        }

        private WorkPermitFortHillsHistory PopulateInstance(SqlDataReader reader)
        {
            ...
            WorkPermitFortHillsHistory history = new WorkPermitFortHillsHistory(id, lastModifiedBy, lastModifiedDate);
            ...
            history.ChemicalSuit = reader.Get<bool>("Goggles");
            history.FaceShield = reader.Get<bool>("FaceShield");
            history.ConfinedSpaceMoniter = reader.Get<bool>("RubberGloves");//**
            history.SuppliedBreathingAir = reader.Get<bool>("RubberSuit");
            history.PersonalFlotationDevice = reader.Get<bool>("HighVoltagePPE");
            history.Other1Checked = reader.Get<bool>("Other1Checked");
            history.Other1 = reader.Get<string>("Other1");
            history.MonoGoggles = reader.Get<bool>("EquipmentGrounded");
        ...
        private void AddInsertParameters(WorkPermitFortHillsHistory history, SqlCommand command)
        {
            ...
            //command.AddParameter("CreatedDateTime", history.CreatedDateTime);
            //command.AddParameter("CreatedByUserId", history.PermitRequestCreatedByUserIdUser);
           ...
           // command.AddParameter("IssuedByUserId", history.IssuedByUser);
           // command.AddParameter("PermitRequestCreatedByUserId", history.PermitRequestCreatedByUserIssuedBy);
            command.AddParameter("PriorityId", history.Priority.IdValue);
           // command.AddParameter("WorkPermitFortHillsId", history.WorkPermitFortHillsId);
            command.AddParameter("EquipmentNo", history.EquipmentNo);
            command.AddParameter("Craft", history.Craft);
            ...
            command.AddParameter("EmergencyAssemblyArea", history.EmergencyAssemblyArea);
            ...
            command.AddParameter("RevalidationDateTime", history.RevalidationDateTime);
            command.AddParameter("ExtensionDateTime", history.ExtensionDateTime);
            ...
            //command.AddParameter("FaceShield", history.FaceShield);
            //command.AddParameter("Goggles", history.ChemicalSuit);
            //command.AddParameter("RubberBoots", history.RubberBoots);
            //command.AddParameter("RubberGloves", history.ConfinedSpaceMoniter);//
            //command.AddParameter("RubberSuit", history.SuppliedBreathingAir);
            //command.AddParameter("SafetyHarnessLifeline", history.SafetyHarnessLifeline);
            //command.AddParameter("HighVoltagePPE", history.PersonalFlotationDevice);
            //command.AddParameter("Other1Checked", history.Other1Checked);
            //command.AddParameter("Other1", history.Other1);
            //command.AddParameter("EquipmentGrounded", history.MonoGoggles);
            //command.AddParameter("FireBlanket", history.FireBlanket);
            //command.AddParameter("FireExtinguisher", history.FireExtinguisher);
