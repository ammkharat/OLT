...
{
    public class GenericCsdDao : AbstractManagedDao, IGenericCsdDao
    {
        private const string QUERY_BY_ID_STORED_PROCEDURE = "QueryFormGenericCsdById";
        private const string INSERT_STORED_PROCEDURE = "InsertFormGenericCsd";
        private const string UPDATE_STORED_PROCEDURE = "UpdateFormGenericCsd";
        private const string INSERT_FORM_FUNCTIONAL_LOCATION = "InsertFormGenericCsdFunctionalLocation";

        private const string DELETE_FORM_FUNCTIONAL_LOCATION =
            "DeleteFormGenericCsdFunctionalLocationsByFormMontrealCsdId";

        private const string INSERT_FORM_APPROVAL = "InsertFormGenericCsdApproval";
        private const string UPDATE_FORM_APPROVAL = "UpdateFormGenericCsdApproval";
        private const string REMOVE_STORED_PROCEDURE = "RemoveFormGenericCsd";

        private const string QUERY_ALL_APPROVED_AND_OUT_OF_SERVICE_FOR_MORE_THAN_3_DAYS =
            "QueryAllFormGenericCsdThatAreApprovedAndOutOfServiceForMoreThan3Days";
 private const string QUERY_ALL_APPROVED_AND_OUT_OF_SERVICE_FOR_MORE_THAN_5_DAYS =
            "QueryAllFormGenericCsdThatAreApprovedAndOutOfServiceForMoreThan5Days";
        ...
        public GenericCsd Insert(GenericCsd form)
        {
        ...
        //ayman generic forms
        public GenericCsd QueryByIdAndSiteId(long id, long siteid)
        {
            var command = ManagedCommand;
            return command.QueryByIdAndSiteId(id, siteid, PopulateInstance, QUERY_BY_ID_STORED_PROCEDURE);
        }
        
        public GenericCsd QueryById(long id)
        {
            var command = ManagedCommand;
            return command.QueryById(id, PopulateInstance, QUERY_BY_ID_STORED_PROCEDURE);
        }

        public List<GenericCsd> QueryAllThatAreApprovedAndAreMoreThan3DaysOutOfService(DateTime now)
        {
            var command = ManagedCommand;
        ...
        public List<GenericCsd> QueryAllThatAreApprovedAndAreMoreThan5DaysOutOfService(DateTime currentTimeAtSite)
        {
            var command = ManagedCommand;
        ...
        public void Update(GenericCsd form)
        {
            var command = ManagedCommand;
        ...
        public void Remove(GenericCsd form)
        {
            var command = ManagedCommand;
        ...
        private void AddUpdateParameters(GenericCsd form, SqlCommand command)
        {
            command.AddParameter("@Id", form.Id);
            SetCommonAttributes(form, command);
        }

        private void UpdateApprovals(SqlCommand command, GenericCsd form)
        {
        ...
        private void UpdateFunctionalLocations(SqlCommand command, GenericCsd form)
        {
        ...
        private void InsertApprovals(SqlCommand command, GenericCsd form)
        {
            if (!form.Approvals.IsEmpty())
            {
        ...
        private void InsertFunctionalLocations(SqlCommand command, GenericCsd form)
        {
        ...
        private void AddInsertParameters(GenericCsd form, SqlCommand command)
        {
            SetCommonAttributes(form, command);
        ...
        private void SetCommonAttributes(GenericCsd form, SqlCommand command)
        {
        ...
        private GenericCsd PopulateInstance(SqlDataReader reader)
        {
...
//            var siteid = reader.Get<long>("SiteId");    //ayman generic forms remove siteid
            ...
            var form = new GenericCsd(id, formStatus, validFromDateTime, validToDateTime, createdBy, createdDateTime)    //ayman generic forms
            {
