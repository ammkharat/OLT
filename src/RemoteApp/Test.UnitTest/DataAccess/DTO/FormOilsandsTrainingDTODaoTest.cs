...
{
    public class FormOilsandsTrainingDTODaoTest : AbstractDaoTest
    {
        ...
        public void QueryByDateRangeAndFLOCShouldOnlyBringBackFormsThatFallUnderOrAboveOrEqualToTheSpecifiedFLOC()
        {
            ...
            {
                List<FormOilsandsTrainingDTO> results = dtoDao.QueryByFunctionalLocationsAndDateRange(new RootFlocSet(floc1), new DateRange(null, null));
            ...
            {
                List<FormOilsandsTrainingDTO> results = dtoDao.QueryByFunctionalLocationsAndDateRange(new RootFlocSet(floc2), new DateRange(null, null));
                Assert.AreEqual(3, results.Count);
                Assert.IsTrue(results.Exists(form => form.Id == form1.Id));
                Assert.IsTrue(results.Exists(form => form.Id == form3.Id));
                Assert.IsTrue(results.Exists(form => form.Id == form4.Id));
            ...
            {
                List<FormOilsandsTrainingDTO> results = dtoDao.QueryByFunctionalLocationsAndDateRange(new RootFlocSet(floc3), new DateRange(null, null));
                Assert.AreEqual(3, results.Count);
            ...
            {
                List<FormOilsandsTrainingDTO> results = dtoDao.QueryByFunctionalLocationsAndDateRange(new RootFlocSet(new List<FunctionalLocation> { floc1, floc3 }), new DateRange(null, null));
        ...
        private void AssertQueryByDateRange(bool exists, Date from, Date to, FormOilsandsTraining form)
        {
            ...
            List<FormOilsandsTrainingDTO> results = dtoDao.QueryByFunctionalLocationsAndDateRange(new RootFlocSet(functionalLocations), new DateRange(@from, to));
